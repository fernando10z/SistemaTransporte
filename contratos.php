...
<div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Contratos</h3>
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalContratos()">
            ...
            <div class="row">
              <div class="col-md-3">
                <div class="form-group">
                  <label for="filtroTipo">Tipo de Contrato</label>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="contratos-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Tipo</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Tipos"></th>
                    ...
                    <th><input type="text" class="column-filter" placeholder="Número Doc."></th>
                ...
                <tbody>
                  ...
                  foreach ($contratos as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-contrato'>{$row['idContrato']}</span></td>";
                    echo "<td><span class='id-contrato'>{$row['tipo']}</span></td>";
                      // Mostrar nombre o razón social según tipo
                      $cliente = $row['tipo'] === 'Natural' ? $row['nombreCliente'] : $row['razonSocial'];
                      echo "<td>{$cliente}</td>";
                  
                      // Tipo de documento o tipo de RUC
                      if ($row['tipo'] === 'Natural') {
                          echo "<td>{$row['tipoDocumento']}</td>";
                          echo "<td>{$row['documento']}</td>";
                      } else {
                          echo "<td>{$row['tipoRuc']}</td>";
                          echo "<td>{$row['ruc']}</td>";
                      }
                      ...
                      echo "<td><span class='badge {$badge}'>{$row['estado']}</span></td>";
                  
                      // Botones de acción
                      echo "<td class='text-center'>";
                      echo "<div class='btn-action-wrapper'>";
                      echo "<button class='btn btn-info btn-icon btn-sm ver-historial' 
                              data-tipo='{$row['tipo']}' 
                      ...
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-contrato'
                              data-id='{$row['idContrato']}'
                              title='Eliminar contrato'>
                              <i class='fas fa-trash'></i>
                            </button>";
                  
                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-contrato'
                              data-id='{$row['idContrato']}'
                              data-estado='{$row['estado']}'
                              title='Cambiar estado'>
                              <i class='fas fa-sync-alt'></i>
                            </button>";
                       echo "<button class='btn btn-sm btn-secondary pdf-contrato' data-id='{$row['idContrato']}' data-tipo='{$row['tipo']}' title='Generar PDF'><i class='fas fa-file-pdf'></i></button>";
...
<form id="pdfForm" action="reportes/generarpdfcontratogeneral.php" method="post" style="display: none;">
 ...
 <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <?php include('contratos/modal.php')?>
  <?php include('contratos/historial.php')?>
  
  <script>
    function abrirModalContratos() {
        if (typeof $('#registrarContratoModal').modal !== 'function') {
        ...
        if (contratoForm) {
          contratoForm.reset();
            ...
            console.warn('Formulario con ID "contratoForm" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#contratos-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-file-contract fa-3x text-muted mb-3"></i><p class="text-muted">No hay contratos para mostrar</p></div>'
    },
    pageLength: 10,
    ...
    initComplete: function() {
      ...
      $('#filtroTipo').on('change', function() {
        table.column(1).search(this.value === 'Juridica' ? 'Empresa' : this.value).draw();
      });
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var data = this.data();
      filteredData.push({
        idContrato: $(data[0]).text(),
                tipo: $(data[1]).text(),
        cliente: data[2],
        tipoDocumento: data[3],
        documento: data[4],
...
$(function() {
  ...
  $(document).on('click', '.eliminar-contrato', function() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.post('contratos/eliminar.php', { idContrato: id }, response => {
  ...
  $(document).on('click', '.cambiar-estado-contrato', function() {
    const id = $(this).data('id');
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.post('contratos/cambiarestado.php', {
          idContrato: id,
...
<script>
 $(document).on('click', '.pdf-contrato', function(e) {
    e.preventDefault();
    
    const idContrato = $(this).data('id');
    const tipoContrato = $(this).data('tipo');
    
    // Mostrar confirmación
    Swal.fire({
        title: 'Generar PDF',
        text: '¿Desea generar el PDF y marcar este contrato como completado?',
        icon: 'question',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            window.open(`reportes/generarpdfcontrato.php?id=${idContrato}&tipo=${tipoContrato}`, '_blank');
            
            // Recargar la página después de 1 segundo para ver el estado actualizado
            setTimeout(() => {
                location.reload();
            }, 1000);
        }
