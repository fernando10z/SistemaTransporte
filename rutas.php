...
<div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Rutas</h3>
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalRutas()">
              ...
              <div class="col-md-9">
                <div class="form-group">
                  ...
                  <div class="filtro-input">
                    ...
                    <input type="text" class="form-control" id="filtroBusquedaGlobal" placeholder="Buscar por zona, origen, destino...">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="rutas-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Zona</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Distancia"></th>
                    <th><input type="text" class="column-filter" placeholder="Tiempo"></th>
                ...
                <tbody>
                  ...
                  foreach ($rutas as $row) {
                      ...
                      if ($row['estado'] === 'Activado') {
                          echo '<td><span class="badge badge-success"><i class="fas fa-check-circle me-1"></i> Activado</span></td>';
                      } else {
                          echo '<td><span class="badge badge-danger"><i class="fas fa-times-circle me-1"></i> Desactivado</span></td>';
                      }

                      // Botones de acción
                      echo "<td>";
                      echo "<div class='btn-action-wrapper'>";
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-ruta' 
                              data-id='{$row['id']}' 
                              title='Editar ruta'>
                              <i class='fas fa-pen'></i>
                            </button>";

                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-ruta'
                              data-id='{$row['id']}'
                              title='Eliminar ruta'>
                              <i class='fas fa-trash'></i>
                            </button>";

                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-ruta'
                              data-id='{$row['id']}'
...
<form id="pdfForm" action="reportes/generarpdfrutas.php" method="post" style="display: none;">
...
$(document).ready(function() {
  ...
  var table = $('#rutas-table').DataTable({
    responsive: true,
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-route fa-3x text-muted mb-3"></i><p class="text-muted">No hay rutas para mostrar</p></div>'
    },
    pageLength: 10,
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var row = this.node();
      var data = {
        id: $(row).find('.id-ruta').text(),
        zona: $(row).find('td:eq(1)').text(),
        origen: $(row).find('td:eq(2)').text(),
        destino: $(row).find('td:eq(3)').text(),
        distancia: $(row).find('td:eq(4)').text(),
        tiempo: $(row).find('td:eq(5)').text(),
        descripcion: $(row).find('td:eq(6)').text(),
