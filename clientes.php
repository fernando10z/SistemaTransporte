...
<body>
            ...
            <div class="card">
              ...
              <div class="card-body">
                <div class="table-responsive table-fade-in">
                  <table id="clientes-table" class="display table table-hover w-100">
                    <thead>
                      <tr>
                        <th width="70px">ID</th>
                        <th>Tipo</th>
                      ...
                      <tr class="filter-header">
                        ...
                        <th><input type="text" class="column-filter" placeholder="Nombre o Razón Social"></th>
                    ...
                    <tbody>
                      ...
                      foreach ($clientes as $row) {
                          ...
                          echo "<button class='btn btn-primary btn-icon btn-sm editar-cliente' 
                                  data-id='{$row['id']}' 
                          ...
                          echo "<button class='btn btn-danger btn-icon btn-sm eliminar-cliente'
                                  data-id='{$row['id']}'
                                  data-tipo='{$row['tipoCliente']}'
                          ...
                          echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-cliente'
                                  data-id='{$row['id']}'
                                  data-tipo='{$row['tipoCliente']}'
    ...
    $(document).ready(function() {
      ...
      var table = $('#clientes-table').DataTable({
        ...
        language: {
          ...
          emptyTable: '<div class="text-center my-5"><i class="fas fa-users fa-3x text-muted mb-3"></i><p class="text-muted">No hay clientes para mostrar</p></div>'
        },
        pageLength: 10,
  ...
  <script>
    ...
    // Cambiar estado cliente con SweetAlert
    $(document).on("click", ".cambiar-estado-cliente", function () {
        const id = $(this).data("id");
        const tipo = $(this).data("tipo");
        const estadoActual = $(this).data("estado");
        const nuevoEstado = estadoActual === 'Activo' ? 'Inactivo' : 'Activo';
        const colorEstado = nuevoEstado === 'Activo' ? '#36b37e' : '#f55252';
        const iconoEstado = nuevoEstado === 'Activo' ? 'check-circle' : 'times-circle';

        Swal.fire({
            title: 'Cambiar estado',
            html: `¿Deseas cambiar el estado del cliente a <span style="color:${colorEstado};font-weight:bold;"><i class="fas fa-${iconoEstado} me-1"></i>${nuevoEstado}</span>?`,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#5d87ff',
            cancelButtonColor: '#78909c',
            confirmButtonText: '<i class="fas fa-sync-alt me-1"></i> Cambiar',
            cancelButtonText: '<i class="fas fa-times me-1"></i> Cancelar',
            buttonsStyling: true,
            customClass: {
    ...
    $(document).on("click", ".editar-cliente", function() {
        ...
        // Cargar los datos del cliente y abrir el modal (el código actual)
        // Aquí iría la lógica para cargar y mostrar el modal de edición
    });
  </script>
</body>
</html>