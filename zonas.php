...
<div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Zonas</h3>
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="card-title">Listado de Zonas de Cobertura</h4>
            <div class="d-flex">
              <button class="btn btn-toggle-filtros me-2" id="toggleFiltros">
                <i class="fas fa-filter"></i>
              </button>
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalZonas()">
            ...
            <div class="row">
                ...
                <div class="form-group">
                  ...
                  <div class="select-container">
                    <select class="form-control" id="filtroDepartamento">
                      ...
                      <?php
                      $sqlDepartamentos = "SELECT DISTINCT departamento FROM zonas_cobertura WHERE departamento IS NOT NULL ORDER BY departamento";
                      $stmtDepartamentos = $conn->prepare($sqlDepartamentos);
                      $stmtDepartamentos->execute();
                      $departamentos = $stmtDepartamentos->fetchAll(PDO::FETCH_COLUMN);
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <div class="select-container">
                    <select class="form-control" id="filtroProvincia">
                      <option value="">Todas</option>
                      <?php
                      $sqlProvincias = "SELECT DISTINCT provincia FROM zonas_cobertura WHERE provincia IS NOT NULL ORDER BY provincia";
                      $stmtProvincias = $conn->prepare($sqlProvincias);
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="zonas-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Nombre Zona</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Departamento"></th>
                    <th><input type="text" class="column-filter" placeholder="Provincia"></th>
                    <th><input type="text" class="column-filter" placeholder="Distrito"></th>
                ...
                <tbody>
                  ...
                  foreach ($zonas as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-zona'>{$row['id']}</span></td>";
                      ...
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-zona me-1' 
                              data-id='{$row['id']}'
                              title='Editar zona'>
                              <i class='fas fa-pen'></i>
                            </button>";
                      
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-zona me-1'
                              data-id='{$row['id']}'
                              title='Eliminar zona'>
                              <i class='fas fa-trash'></i>
                            </button>";
                      
                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-zona'
                              data-id='{$row['id']}'
...
<form id="pdfForm" action="reportes/generarpdfzonas.php" method="post" style="display: none;">
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <?php include('zonas/modal.php')?>
  <?php include('zonas/modaleditar.php')?>
  
  <script>
    function abrirModalZonas() {
        if (typeof $('#registrarZonaModal').modal !== 'function') {
        ...
        const registrarZonaForm = document.getElementById('registrarZonaForm');
        if (registrarZonaForm) {
          registrarZonaForm.reset();
        } else {
            console.warn('Formulario con ID "registrarZonaForm" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#zonas-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-map-marked-alt fa-3x text-muted mb-3"></i><p class="text-muted">No hay zonas registradas</p></div>'
    },
    pageLength: 10,
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var row = this.node();
      var data = {
        id: $(row).find('.id-zona').text(),
        nombre: $(row).find('td:eq(1)').text(),
        departamento: $(row).find('td:eq(2)').text(),
        provincia: $(row).find('td:eq(3)').text(),
        distrito: $(row).find('td:eq(4)').text(),
        descripcion: $(row).find('td:eq(5)').text(),
...
function initTooltips() {
  ...
  <script>
    // Eliminar zona con SweetAlert
$(document).on('click', '.eliminar-zona', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('zonas/eliminar.php', { id }, function (response) {
...
// Cambiar estado de la zona con SweetAlert
$(document).on('click', '.cambiar-estado-zona', function () {
    const id = $(this).data('id');

    $.post('zonas/cambiarestado.php', { id }, function (response) {
        const res = JSON.parse(response);
