        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalEnvios()">
          ...
          <div class="filtros-avanzados" id="filtrosAvanzados">
            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label for="filtroTipo">Tipo de Envío</label>
                  <div class="select-container">
                    <select class="form-control" id="filtroTipo">
                      <option value="">Todos</option>
                      <option value="cliente">Cliente Natural</option>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div class="filtro-input">
                    ...
                    <input type="text" class="form-control" id="filtroBusquedaGlobal" placeholder="Buscar por remitente, origen, destino...">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="eventos-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Tipo</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Remitente"></th>
                ...
                <tbody>
                  ...
                  $sql = "
                      ...
                      JOIN cotizaciones_empresas ce ON ac.idCotizacionEmpresa = ce.idCotizacionEmpresa
                      JOIN solicitud_empresa s ON ce.idSolicitudempresa = s.idSolicitudempresa
                  ...
                  foreach ($eventos as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-evento'>{$row['idEvento']}</span></td>";
                      
                      // Tipo de envío con icono
                      if ($row['tipoEnvios'] == 'cliente') {
                      ...
                      echo "<div class='btn-action-wrapper'>";
                       echo "<button class='btn btn-sm btn-primary editar' data-id='{$row['idEvento']}' data-type='{$row['tipoEnvios']}'><i class='fas fa-edit'></i></button> ";
    $disabled = $row['estado'] === 'Entregado' ? 'disabled' : '';
    echo "<button class='btn btn-sm btn-warning cambiar-estado' data-id='{$row['idEvento']}' data-type='{$row['tipoEnvios']}' {$disabled}><i class='fas fa-sync'></i></button> ";
    echo "<button class='btn btn-sm btn-danger eliminar' data-id='{$row['idEvento']}' data-type='{$row['tipoEnvios']}'><i class='fas fa-trash'></i></button>";
                      echo "</div>";
                      echo "</td>";
                      echo "</tr>";
                  }
...
<form id="pdfForm" action="reportes/generarpdfenvios.php" method="post" style="display: none;">
  ...
  <?php include('envios/modal.php')?>
  <?php include('envios/modaleditar.php')?>

  
 
  <script>
    function abrirModalEnvios() {
        if (typeof $('#modalEventoEnvio').modal !== 'function') {
            console.error('Bootstrap Modal no está cargado correctamente');
            alert('Error al cargar el sistema. Recarga la página.');
            return;
        }
        
        $('#modalEventoEnvio').modal('show');
        
        const formEventoEnvio = document.getElementById('formEventoEnvio');
        if (formEventoEnvio) {
          formEventoEnvio.reset();
        } else {
            console.warn('Formulario con ID "formEventoEnvio" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#eventos-table').DataTable({
    responsive: true,
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-calendar-alt fa-3x text-muted mb-3"></i><p class="text-muted">No hay eventos registrados</p></div>'
    },
    pageLength: 10,
    ...
    initComplete: function() {
      ...
      $('#filtroTipo').on('change', function() {
        table.column(1).search(this.value === 'cliente' ? 'Natural' : 'Empresa').draw();
      });
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      ...
      filteredData.push({
        idEvento: $row.find('.id-evento').text(),
        tipoEnvios: $(rowData[1]).text().trim(),
        remitente: rowData[2],
        origen: rowData[3],
        destino: rowData[4],
        fechaEvento: rowData[5],
        estado: $(rowData[6]).text().trim()
      });
    });
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".cambiar-estado").forEach(button => {
        button.addEventListener("click", function () {
            ...
            const tipoEnvios = this.dataset.type;
            ...
            Swal.fire({
                ...
                preConfirm: (nuevoEstado) => {
                    return fetch('envios/cambiarestado.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        },
                        body: `idEvento=${idEvento}&tipoEnvios=${tipoEnvios}&nuevoEstado=${nuevoEstado}`
                    })
