 ...
 <div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Cursos</h3>
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalCursos()">
              ...
              <div class="col-md-9">
                <div class="form-group">
                  ...
                  <div class="filtro-input">
                    ...
                    <input type="text" class="form-control" id="filtroBusquedaGlobal" placeholder="Buscar por nombre, entidad, descripción...">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="cursos-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Entidad"></th>
                ...
                <tbody>
                  <?php
                  $sql = "SELECT * FROM cursos_conductor ORDER BY fecharegistro DESC";
                  ...
                  foreach ($cursos as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-curso'>{$row['idCurso']}</span></td>";
                      ...
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-curso' 
                              data-id='{$row['idCurso']}'
                              title='Editar curso'>
                              <i class='fas fa-pen'></i>
                            </button>";
                      
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-curso'
                              data-id='{$row['idCurso']}'
                              title='Eliminar curso'>
                              <i class='fas fa-trash'></i>
                            </button>";
                      
                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-curso'
                              data-id='{$row['idCurso']}'
...
<form id="pdfForm" action="reportes/generarpdfcursos.php" method="post" style="display: none;">
...
<?php include('cursos/modal.php')?>
    <?php include('cursos/modaleditar.php')?>
  <script>
    function abrirModalCursos() {
        if (typeof $('#cursoModal').modal !== 'function') {
        ...
        if (cursoForm) {
          cursoForm.reset();
            ...
            console.warn('Formulario con ID "cursoForm" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#cursos-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-graduation-cap fa-3x text-muted mb-3"></i><p class="text-muted">No hay cursos registrados</p></div>'
    },
    pageLength: 10,
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.cambiar-estado-curso').forEach(btn => {
        btn.addEventListener('click', function () {
            const id = this.dataset.id;
            fetch('cursos/cambiarestado.php', {
                method: 'POST',
                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                body: `idCurso=${id}`
            })
