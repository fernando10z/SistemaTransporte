...
<head>
  ...
  <title>Gestión de Eventos Ruta</title>
...
<div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Eventos Rutas</h3>
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Eventos Rutas</li>
          </ol>
        </nav>
      </div>
    </div>
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="card-title">Listado de Eventos Rutas</h4>
            <div class="d-flex">
              <button class="btn btn-toggle-filtros me-2" id="toggleFiltros">
                <i class="fas fa-filter"></i>
              </button>
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalEventos()">
          ...
          <div class="filtros-avanzados" id="filtrosAvanzados">
            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label for="filtroTipo">Tipo de Planificación</label>
                  <div class="select-container">
...
$disabledEditar = $row['estado'] === 'Completado' ? 'disabled' : '';
echo "<button class='btn btn-sm btn-primary editar' data-id='{$row['idEvento']}' data-type='{$row['tipoPlanificacion']}' {$disabledEditar}><i class='fas fa-edit'></i></button> ";

    // Si el estado es 'Completado', deshabilitamos el botón cambiar estado
    $disabled = $row['estado'] === 'Completado' ? 'disabled' : '';
    echo "<button class='btn btn-sm btn-warning cambiar-estado' data-id='{$row['idEvento']}' data-type='{$row['tipoPlanificacion']}' {$disabled}><i class='fas fa-sync'></i></button> ";

    echo "<button class='btn btn-sm btn-danger eliminar' data-id='{$row['idEvento']}' data-type='{$row['tipoPlanificacion']}'><i class='fas fa-trash'></i></button>";
                      echo "</div>";
                      echo "</td>";
                      echo "</tr>";
                  }
...
<form id="pdfForm" action="reportes/generarpdfeventos.php" method="post" style="display: none;">
  ...
  <?php include('eventos/modal.php')?>
  <?php include('eventos/modaleditar.php')?>

  
 
  <script>
    function abrirModalEventos() {
        if (typeof $('#modalEventoRuta').modal !== 'function') {
            console.error('Bootstrap Modal no está cargado correctamente');
            alert('Error al cargar el sistema. Recarga la página.');
            return;
        }
        
        $('#modalEventoRuta').modal('show');
        
        const formEventoRuta = document.getElementById('formEventoRuta');
        if (formEventoRuta) {
          formEventoRuta.reset();
        } else {
            console.warn('Formulario con ID "formEventoRuta" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#eventos-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-route fa-3x text-muted mb-3"></i><p class="text-muted">No hay eventos de ruta registrados</p></div>'
    },
    pageLength: 10,
    ...
    drawCallback: function() {
      ...
      // Deshabilitar botones de cambiar estado para eventos completados
      $('tr').each(function() {
        var estado = $(this).find('td:eq(7)').text().trim();
        if (estado === 'Completado') {
          $(this).find('.cambiar-estado').prop('disabled', true);
        }
      });
    }
  });
  ...
  $(document).on('click', '.cambiar-estado', function() {
    var id = $(this).data('id');
    var type = $(this).data('type');
    // Lógica para cambiar estado del evento
  });
...
$(document).on('click', '.cambiar-estado', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed && result.value) {
            $.post('eventos/cambiarestado.php', {
                idEvento: id,
                tipo: tipo,
                nuevoEstado: result.value
            }, function (response) {
                Swal.fire('¡Actualizado!', response, 'success').then(() => location.reload());
            });
        }
    });
});
