        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalProveedores()">
              ...
              <div class="col-md-9">
                <div class="form-group">
                  ...
                  <div class="filtro-input">
                    ...
                    <input type="text" class="form-control" id="filtroBusquedaGlobal" placeholder="Buscar por empresa, RUC, contacto...">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="proveedores-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Empresa</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Empresa"></th>
                    <th><input type="text" class="column-filter" placeholder="Tipo RUC"></th>
                    <th><input type="text" class="column-filter" placeholder="N° RUC"></th>
                    <th><input type="text" class="column-filter" placeholder="Contacto"></th>
                ...
                <tbody>
                  ...
                  foreach ($proveedores as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-proveedor'>{$row['idProveedor']}</span></td>";
                      echo "<td>{$row['nombre_empresa']}</td>";
                      echo "<td>{$row['tipo_ruc']}</td>";
                      echo "<td>{$row['numero_ruc']}</td>";
                      echo "<td>{$row['contacto_nombre']}</td>";
                      echo "<td>{$row['contacto_telefono']}</td>";
                      echo "<td>{$row['contacto_correo']}</td>";
                      echo "<td>{$row['tipoDireccion']} - {$row['direccion']}</td>";
                      
                      // Estado con badge
                      $badgeClass = $row['estado'] == 'Activo' ? 'bg-success' : 'bg-danger';
                      echo "<td class='text-center'><span class='badge {$badgeClass}'>{$row['estado']}</span></td>";
                      
                      // Botones de acción
                      echo "<td class='text-center'>";
                      echo "<div class='btn-action-wrapper'>";
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-proveedor' 
                              data-id='{$row['idProveedor']}'
                              title='Editar proveedor'>
                              <i class='fas fa-edit'></i>
                            </button>";
                      
                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-proveedor'
                              data-id='{$row['idProveedor']}'
                      ...
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-proveedor'
                              data-id='{$row['idProveedor']}'
...
<form id="pdfForm" action="reportes/generarproveedor.php" method="post" style="display: none;">
  ...
  <?php include('proveedores/modal.php')?>
  <?php include('proveedores/modaleditar.php')?>
  
  <script>
    function abrirModalProveedores() {
        if (typeof $('#modalProveedor').modal !== 'function') {
        ...
        if (formProveedor) {
          formProveedor.reset();
            ...
            console.warn('Formulario con ID "formProveedor" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#proveedores-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-truck fa-3x text-muted mb-3"></i><p class="text-muted">No hay proveedores registrados</p></div>'
    },
    pageLength: 10,
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var data = this.data();
      filteredData.push({
        idProveedor: $(data[0]).text(),
        nombre_empresa: data[1],
        tipo_ruc: data[2],
        numero_ruc: data[3],
        contacto_nombre: data[4],
        contacto_telefono: data[5],
        contacto_correo: data[6],
        direccion: data[7],
        estado: $(data[8]).text()
      });
    });
...
document.addEventListener('DOMContentLoaded', function () {
    // Cambiar estado
    document.querySelectorAll('.cambiar-estado-proveedor').forEach(button => {
        button.addEventListener('click', () => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`proveedores/cambiarestado.php?id=${id}&estado=${nuevoEstado}`)
                        .then(res => res.text())
                        .then(data => {
                            Swal.fire('¡Actualizado!', data, 'success').then(() => location.reload());
                        });
                }
            });
        });
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.eliminar-proveedor').forEach(button => {
        button.addEventListener('click', function () {
            ...
            Swal.fire({
                title: '¿Estás seguro?',
                text: "Esta acción eliminará al proveedor y su firma asociada.",
                icon: 'warning',
                showCancelButton: true,
