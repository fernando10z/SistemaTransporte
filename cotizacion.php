...
<div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Cotizaciones</h3>
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalCotizacion()">
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="cotizaciones-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Tipo Cliente</th>
                ...
                <tbody>
                  ...
                  $sql = "
                  SELECT 
                    cc.idCotizacion AS idCotizacion,
                    'cliente' AS tipoCotizacion,
                  ...
                  SELECT 
                    ce.idCotizacionEmpresa AS idCotizacion,
                    'empresa' AS tipoCotizacion,
                    'Empresa' AS tipoCliente,
                    cemp.razonSocial AS cliente,
                    se.origen,
                    se.destino,
                    se.peso,
                    se.volumen,
                  ...
                  foreach ($cotizaciones as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-cotizacion'>{$row['idCotizacion']}</span></td>";
                      ...
                      $badgeClass = match(strtolower($row['estado'])) {
                          'pendiente' => 'warning',
                          'aceptada' => 'success',
                          'rechazada' => 'danger',
                          default => 'secondary'
                  ...
                  // Deshabilita botones si el estado es 'aceptada' o 'rechazada'
    $disabled = in_array(strtolower($row['estado']), ['aceptada', 'rechazada']) ? 'disabled' : '';
                      echo "<td class='text-center'>";
                      echo "<div class='btn-action-wrapper'>";
                        echo "<button class='btn btn-sm btn-primary me-1 editar' data-id='{$row['idCotizacion']}' title='Editar' $disabled>
            <i class='fas fa-edit'></i>
          </button>";

    // Botón Cambiar Estado
    echo "<button class='btn btn-sm btn-warning cambiar-estado me-1' data-id='{$row['idCotizacion']}' data-type='{$row['tipoCotizacion']}' title='Cambiar Estado' $disabled>
            <i class='fas fa-sync-alt'></i>
          </button>";

                       echo "<button class='btn btn-sm btn-danger eliminar' data-id='{$row['idCotizacion']}' data-type='{$row['tipoCotizacion']}' title='Eliminar'><i class='fas fa-trash'></i></button>";
...
<form id="pdfForm" action="reportes/generarpdfcotizaciones.php" method="post" style="display: none;">
 ...
 <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <?php include('cotizacion/modal.php')?>
  <?php include('cotizacion/modaleditar.php')?>
  
  <script>
// Función para abrir el modal principal
function abrirModalCotizacion() {
        if (typeof $('#cotizacionModal').modal !== 'function') {
        ...
        const formCotizacionCliente = document.getElementById('formCotizacionCliente');
        if (formCotizacionCliente) {
          formCotizacionCliente.reset();
        } else {
            console.warn('Formulario con ID "formCotizacionCliente" no encontrado');
        }
    }

// Configurar el modal principal para que no se abra automáticamente
...
$(document).ready(function() {
  ...
  var table = $('#cotizaciones-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-file-invoice-dollar fa-3x text-muted mb-3"></i><p class="text-muted">No hay cotizaciones para mostrar</p></div>'
    },
    pageLength: 10,
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      ...
      filteredData.push({
        ...
        peso: data[5],
        volumen: data[6],
        montoFinal: data[7].replace('S/ ', ''),
        fechaCotizacion: data[8],
        estado: $(data[9]).text()
      });
    });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.cambiar-estado').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            Swal.fire({
                title: '¿Estás seguro?',
                text: "¿Deseas marcar esta cotización como Aceptada?",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    fetch('cotizacion/cambiarestado.php', {
                        method: 'POST',
                    ...
                    .then(res => res.json())
                    .then(data => {
                        Swal.fire('Estado cambiado', data.message, 'success').then(() => location.reload());
                    })
