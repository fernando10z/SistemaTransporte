...
<body>
  ...
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Asistencias</h3>
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalAsistencia()">
              ...
              <div class="col-md-8">
                <div class="form-group">
                  ...
                  <div class="filtro-input">
                    ...
                    <input type="text" class="form-control" id="filtroBusquedaGlobal" placeholder="Buscar conductor, documento, etc...">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="asistencias-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Conductor</th>
                    <th>Documento</th>
                    <th width="100px">Horas Trabajo</th>
                    <th width="100px">Día</th>
                    <th width="100px">Hora Entrada</th>
                    <th width="100px">Hora Salida</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Día"></th>
                    <th><input type="text" class="column-filter" placeholder="Entrada"></th>
                    <th><input type="text" class="column-filter" placeholder="Salida"></th>
                ...
                <tbody>
                  ...
                  foreach ($asistencias as $row) {
                      ...
                      // Estado (badge verde o gris)
                      $badgeClass = ($row['estado'] === 'Ingreso') ? 'success' : 'secondary';
                      echo "<td class='text-center'><span class='badge bg-{$badgeClass}'>{$row['estado']}</span></td>";

                      // Botones
                      echo "<td class='text-center'>";
                      echo "<div class='btn-group btn-group-sm'>";
                      echo "<button class='btn btn-info ver-asistencia' data-id='{$row['idConductor']}' title='Ver'>
                              <i class='fas fa-eye'></i>
                            </button>";
                      echo "<button class='btn btn-danger eliminar-asistencia' data-id='{$row['id']}' title='Eliminar'>
...
<form id="pdfForm" action="reportes/generarpdfasistencias.php" method="post" style="display: none;">
...
$(document).ready(function() {
  ...
  var table = $('#asistencias-table').DataTable({
    responsive: true,
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-user-clock fa-3x text-muted mb-3"></i><p class="text-muted">No hay asistencias registradas</p></div>'
    },
    pageLength: 5,
    ...
    initComplete: function() {
      ...
      this.api().columns().every(function(colIdx) {
        if (colIdx === 8) return; // No aplicar filtro a la columna "Acciones"
        var column = this;
    ...
    columnDefs: [
      { targets: [7], className: 'text-center' }, // Estado centrado
      { targets: [8], orderable: false, searchable: false, className: 'text-center' } // Acciones
    ],
...
$(document).on('click','.eliminar-asistencia',function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('eliminar_asistencia.php', { id }, function (resp) {
...
<script>
// Función para obtener hora de Lima
function getHoraLima() {
...
// Consultar vía AJAX si el usuario es administrador
fetch('conexion/auth2.php')
    .then(response => response.json())
    .then(data => {
        if (data.is_admin) {
            const horaLima = getHoraLima();
            const [hora, minutos] = horaLima.split(":").map(Number);

            if (hora < 12 || (hora === 12 && minutos < 59)) {
                Swal.fire({
                    icon: 'info',
                    title: '¡Aviso importante!',
                    text: 'Se tiene que recargar la página para que registre los que no asistieron desde las 12:00pm hasta las 12:59pm',
                    confirmButtonText: 'Entendido',
                    confirmButtonColor: '#3085d6'
                });
            } else if ((hora < 18) || (hora === 18 && minutos < 59)) {
                Swal.fire({
                    icon: 'info',
                    title: '¡Aviso importante!',
                    text: 'Se tiene que recargar la página para que registre los que no asistieron desde las 6:00pm hasta las 6:59pm',
                    confirmButtonText: 'Entendido',
                    confirmButtonColor: '#3085d6'
                });
            }
