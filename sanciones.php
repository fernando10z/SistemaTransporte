...
<div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Sanciones</h3>
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="card-title">Listado de Sanciones</h4>
            <div class="d-flex">
              <button class="btn btn-toggle-filtros me-2" id="toggleFiltros">
                <i class="fas fa-filter"></i>
              </button>
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalSanciones()">
          ...
          <div class="filtros-avanzados" id="filtrosAvanzados">
            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label for="filtroTipo">Tipo de Sanción</label>
                  <div class="select-container">
                    <select class="form-control" id="filtroTipo">
                      <option value="">Todos</option>
                      <?php
                      // Obtener tipos de sanción únicos
                      $sqlTipos = "SELECT DISTINCT tipo_sancion FROM sanciones_conductores";
                      $stmtTipos = $conn->query($sqlTipos);
                      while ($tipo = $stmtTipos->fetch(PDO::FETCH_ASSOC)) {
                        echo "<option value='{$tipo['tipo_sancion']}'>{$tipo['tipo_sancion']}</option>";
                      }
                      ?>
                    </select>
                  </div>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div class="filtro-input">
                    ...
                    <input type="text" class="form-control" id="filtroBusquedaGlobal" placeholder="Buscar por conductor, documento, licencia...">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="sanciones-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Conductor</th>
                ...
                <tbody>
                  ...
                  foreach ($sanciones as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-sancion'>{$row['idSancion']}</span></td>";
                      echo "<td>{$row['nombre_completo']}</td>";
                      echo "<td>{$row['documento_completo']}</td>";
                      echo "<td>{$row['licencia_completa']}</td>";
                    ...
                    echo "<td><span class='badge {$badgeClass} text-white'><i class='fas fa-circle me-1'></i> {$row['estado']}</span></td>";
                      ...
                      // Botón Ver
                      echo "<button type='button' class='btn btn-info btn-icon btn-sm btnVerSancion me-1' 
                            data-idconductor='{$row['idSancion']}' title='Ver Detalles'>
                            <i class='fas fa-eye'></i>
                            </button>";

                      // Botón Cambiar Estado
                      echo "<button type='button' class='btn btn-warning btn-icon btn-sm btnCambiarEstado me-1' 
                              data-id='{$row['idSancion']}' title='Cambiar Estado'>
                              <i class='fas fa-exchange-alt'></i>
                            </button>";

                      // Botón Eliminar
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-sancion' 
                              data-id='{$row['idSancion']}' title='Eliminar'>
...
<form id="pdfForm" action="reportes/generarpdfsanciones.php" method="post" style="display: none;">
  ...
  <?php include('sanciones/modal.php')?>
    <?php include('sanciones/historial.php')?>
  
  <script>
    function abrirModalSanciones() {
        if (typeof $('#modalRegistrarSancion').modal !== 'function') {
        ...
        if (formSancion) {
          formSancion.reset();
            ...
            console.warn('Formulario con ID "formSancion" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#sanciones-table').DataTable({
    responsive: true,
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-exclamation-triangle fa-3x text-muted mb-3"></i><p class="text-muted">No hay sanciones registradas</p></div>'
    },
    pageLength: 10,
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var row = this.node();
      var data = {
        idSancion: $(row).find('.id-sancion').text(),
        conductor: $(row).find('td:eq(1)').text(),
        documento: $(row).find('td:eq(2)').text(),
        licencia: $(row).find('td:eq(3)').text(),
        tipo_sancion: $(row).find('td:eq(4)').text(),
        fecha_registro: $(row).find('td:eq(5)').text(),
...
<script>
    // Eliminar sanción
    $(document).on('click', '.eliminar-sancion', function() {
        var id = $(this).data('id');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('sanciones/eliminar.php', { id: id }, function(response) {
                    let res = JSON.parse(response);
                    Swal.fire(res.message, '', res.success ? 'success' : 'error').then(() => {
    ...
    $(document).on('click', '.btnCambiarEstado', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('sanciones/cambiarestado.php', { id: id }, function(response) {
                    let res = JSON.parse(response);
                    Swal.fire(res.message, '', res.success ? 'success' : 'error').then(() => {
                        if (res.success) location.reload();
                    });
