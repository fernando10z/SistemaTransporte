...
try {
    ...
    $sqlAnular = "UPDATE reprogramacionesEmpresa 
                 SET estado = 'Anulado'
                 WHERE idPlanificacionempresa = :id AND estado IN ('Activo', 'Pendiente')";
    $stmtAnular = $conn->prepare($sqlAnular);
    $stmtAnular->execute([':id' => $_POST['idPlanificacion']]);

    // 2. Insertar nueva reprogramación
    $sqlInsert = "INSERT INTO reprogramacionesEmpresa (
        idPlanificacionempresa, Planificacionoriginal, motivo, 
    ...
    $sqlUpdate = "UPDATE planificacion_ruta_empresa 
