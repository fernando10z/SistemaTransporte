        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalTransferencia()">
            ...
            <div class="row">
              <div class="col-md-3">
                <div class="form-group">
                  <label for="filtroTipo">Tipo de Transacción</label>
                ...
                <div class="form-group">
                  ...
                  <div class="select-container">
                    <select class="form-control" id="filtroMetodo">
                      ...
                      $sqlMetodos = "SELECT DISTINCT metodo_pago FROM transacciones_financieras WHERE metodo_pago IS NOT NULL AND metodo_pago != ''";
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="transacciones-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Concepto"></th>
                ...
                <tbody>
                  ...
                  ORDER BY tf.fecha_registro DESC
                  ";
                  ...
                  foreach ($transacciones as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-transaccion'>{$row['idtransaccion']}</span></td>";
                      
                      // Tipo de transacción con badge e icono
                      $tipoIcono = $row['tipo'] === 'Ingreso' ? 'fa-arrow-down text-success' : 'fa-arrow-up text-danger';
                      echo "<td><span class='badge bg-" . ($row['tipo'] === 'Ingreso' ? 'success' : 'danger') . "'>
                            <i class='fas {$tipoIcono} me-1'></i> {$row['tipo']}
                            </span></td>";
                      
                      echo "<td>" . ($row['nombreServicio'] ?? '<em class="text-muted">Sin servicio</em>') . "</td>";
                      ...
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-transaccion me-1' 
                              data-id='{$row['idtransaccion']}'
                              title='Editar transacción'>
                              <i class='fas fa-pen'></i>
                            </button>";
                      
                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-transaccion me-1'
                              data-id='{$row['idtransaccion']}'
                      ...
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-transaccion'
                              data-id='{$row['idtransaccion']}'
...
<form id="pdfForm" action="reportes/generarpdftranferencia.php" method="post" style="display: none;">
  ...
  <?php include('transferencia/modal.php')?>
  <?php include('transferencia/modaleditar.php')?>
  
  <script>
    function abrirModalTransferencia() {
        if (typeof $('#modalTransaccionFinanciera').modal !== 'function') {
            console.error('Bootstrap Modal no está cargado correctamente');
            alert('Error al cargar el sistema. Recarga la página.');
            return;
        }
        
        $('#modalTransaccionFinanciera').modal('show');
        
        const formTransaccionFinanciera = document.getElementById('formTransaccionFinanciera');
        if (formTransaccionFinanciera) {
          formTransaccionFinanciera.reset();
        } else {
            console.warn('Formulario con ID "formTransaccionFinanciera" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#transacciones-table').DataTable({
    responsive: true,
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-exchange-alt fa-3x text-muted mb-3"></i><p class="text-muted">No hay transacciones registradas</p></div>'
    },
    pageLength: 10,
    ...
    initComplete: function() {
      ...
      $('#filtroMetodo').on('change', function() {
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var row = this.node();
      var data = {
        id: $(row).find('.id-transaccion').text(),
        tipo: $(row).find('td:eq(1)').text().trim(),
        servicio: $(row).find('td:eq(2)').text(),
        concepto: $(row).find('td:eq(3)').text(),
...
function initTooltips() {
 ...
 <script>
  // Cambiar estado
$(document).on('click', '.cambiar-estado-transaccion', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('transferencia/cambiarestado.php', { id: id, estado: nuevoEstado }, function (response) {
                Swal.fire('Actualizado', 'Estado cambiado correctamente', 'success').then(() => location.reload());
            }).fail(() => {
...
$(document).on('click', '.eliminar-transaccion', function () {
    const id = $(this).data('id');
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('transferencia/eliminar.php', { id: id }, function (response) {
                Swal.fire('Eliminado', 'Transacción eliminada correctamente', 'success').then(() => location.reload());
            }).fail(() => {
