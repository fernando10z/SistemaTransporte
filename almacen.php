...
<body>
  ...
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Almacenes</h3>
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalAlmacen()">
            ...
            <div class="row">
              <div class="col-md-3">
                <div class="form-group">
                  <label for="filtroTipo">Tipo de Almacenes</label>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="almacenes-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="80px">ID</th>
                    <th>Nombre</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Ubicación"></th>
                ...
                <tbody>
                  ...
                  foreach ($almacenes as $row) {
                      ...
                      echo "<button class='btn btn-primary editar-almacen' data-id='{$row['idAlmacen']}' title='Editar'>
                              <i class='fas fa-pen'></i>
                            </button>";
                      echo "<button class='btn btn-danger eliminar-almacen' data-id='{$row['idAlmacen']}' title='Eliminar'>
                              <i class='fas fa-trash'></i>
                            </button>";
                      echo "<button class='btn btn-warning cambiar-estado-almacen' data-id='{$row['idAlmacen']}' title='Cambiar estado'>
...
<form id="pdfForm" action="reportes/generarpdfalmacen.php" method="post" style="display: none;">
  ...
  <?php include('almacen/modal.php')?>
  <?php include('almacen/modaleditar.php')?>
  
  <script>
    function abrirModalAlmacen() {
        if (typeof $('#almacenModal').modal !== 'function') {
        ...
        if (almacenForm) {
          almacenForm.reset();
            ...
            console.warn('Formulario con ID "almacenForm" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#almacenes-table').DataTable({
    responsive: true,
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-warehouse fa-3x text-muted mb-3"></i><p class="text-muted">No hay almacenes para mostrar</p></div>'
    },
    pageLength: 5, // Mostrar 5 registros por página inicialmente
    ...
    initComplete: function() {
      // Personalizar el select de cantidad de registros
      ...
      this.api().columns().every(function(colIdx) {
        if (colIdx === 5) return; // No aplicar filtro a la columna "Acciones"
        var column = this;
    ...
    columnDefs: [
      { targets: [4], className: 'text-center' }, // Estado centrado
      { targets: [5], orderable: false, searchable: false } // Acciones no ordenables/buscables
    ],
...
$(document).ready(function () {
    // Cambiar estado
    $(document).on('click', '.cambiar-estado-almacen', function () {
        let id = $(this).data('id');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('almacen/cambiarestado.php', { idAlmacen: id }, function (response) {
                    Swal.fire({
                        icon: 'success',
    ...
    $(document).on('click', '.eliminar-almacen', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('almacen/eliminar.php', { idAlmacen: id }, function (response) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Eliminado',
                        text: response,
