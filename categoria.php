        ...
        <div class="card">
              ...
              <div class="col-md-8">
                <div class="form-group">
                  ...
                  <div class="filtro-input">
                    ...
                    <input type="text" class="form-control" id="filtroBusquedaGlobal" placeholder="Buscar categoría, almacén, etc...">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="categorias-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Almacén</th>
                ...
                <tbody>
                  ...
                  foreach ($categorias as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-categoria'>{$row['idCategoria']}</span></td>";
                      ...
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-categoria' 
                              data-id='{$row['idCategoria']}' 
                              title='Editar categoría'>
                              <i class='fas fa-pen'></i>
                            </button>";
                  
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-categoria'
                              data-id='{$row['idCategoria']}'
                              title='Eliminar categoría'>
                              <i class='fas fa-trash'></i>
                            </button>";
                  
                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-categoria'
                              data-id='{$row['idCategoria']}'
...
<form id="pdfForm" action="reportes/generarpdfcategoria.php" method="post" style="display: none;">
 ...
 <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <?php include('categoria/modal.php')?>
  
  <script>
    function abrirModalCategoria() {
        if (typeof $('#agregarCategoriaModal').modal !== 'function') {
        ...
        if (formAgregarCategoria) {
          formAgregarCategoria.reset();
            ...
            console.warn('Formulario con ID "formAgregarCategoria" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#categorias-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-tags fa-3x text-muted mb-3"></i><p class="text-muted">No hay categorías para mostrar</p></div>'
    },
    pageLength: 10,
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Cambiar estado
    document.querySelectorAll('.cambiar-estado-categoria').forEach(button => {
        button.addEventListener('click', function () {
            const id = this.dataset.id;

            fetch('categoria/cambiarestado.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
            ...
            .then(res => res.text())
            .then(response => {
                Swal.fire('Estado actualizado', response, 'success')
                    .then(() => location.reload());
            });
        });
    });
