...
$planificaciones = isset($_POST['filteredData']) ? json_decode($_POST['filteredData'], true) : [];

// Verificar si los datos llegaron correctamente
if (!$planificaciones || empty($planificaciones)) {
...
$html = '<style>
    ...
    .stat-box { width: 24%; padding: 10px; background-color: #e9ecef; border-radius: 5px; text-align: center; }
</style>';
    ...
    <tr>
        ...
        <td width="20%" style="text-align: right;">
            <div class="report-box">
                <h4>REPORTE DE PLANIFICACIONES</h4>
                <h4>' . date('d/m/Y') . '</h4>
            </div>
        </td>
    </tr>
...
// Resumen estadístico
$totalPlanificaciones = count($planificaciones);
$planificacionesCliente = 0;
$planificacionesEmpresa = 0;
$estados = [
    'Planificado' => 0,
    'Reprogramado' => 0,
    'Cancelado' => 0,
...
foreach ($planificaciones as $plan) {
    $tipoRaw = strtolower(trim($plan['tipoPlanificacion'] ?? ''));

    if ($tipoRaw === 'cliente') {
        $planificacionesCliente++;
    } elseif ($tipoRaw === 'empresa') {
        $planificacionesEmpresa++;
    }

    
    $estado = trim($plan['estado']);
...
foreach ($planificaciones as $plan) {
    ...
    // ID
    $html .= '<td>' . htmlspecialchars($plan['idPlanificacion']) . '</td>';
    ...
    // Origen
    $html .= '<td>' . htmlspecialchars($plan['origen']) . '</td>';
    
    // Destino
    $html .= '<td>' . htmlspecialchars($plan['destino']) . '</td>';
    
    // Fecha y Hora
    $html .= '<td>' . htmlspecialchars($plan['fechaHora']) . '</td>';
    
    // Estado
    $estado = trim($plan['estado']);
    $badgeClass = match ($estado) {
        'Planificado' => 'badge-primary',
...
header('Content-Disposition: attachment; filename="Reporte_Planificaciones_' . date('Ymd_His') . '.pdf"');
echo $dompdf->output();
exit;
?>