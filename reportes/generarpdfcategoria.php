...
$categorias = isset($_POST['filteredData']) ? json_decode($_POST['filteredData'], true) : [];
...
$html .= '<table width="100%">
    <tr>
        ...
        <td width="60%" style="text-align: center;">
            <div class="header">
                <h2 class="title">SISTEMA DE GESTIÓN DE CATEGORÍAS</h2>
                <p class="subtitle">Reporte de Categorías de Productos</p>
            </div>
        </td>
...
foreach ($categorias as $categoria) {
    if (strpos($categoria[5], 'Activo') !== false) {
        $activos++;
    } else {
        $inactivos++;
    }
...
foreach ($categorias as $categoria) {
    ...
    // ID
    $html .= '<td>' . htmlspecialchars($categoria[0]) . '</td>';
    
    // Almacén
    $html .= '<td>' . htmlspecialchars($categoria[1]) . '</td>';
    
    // Categoría
    $html .= '<td>' . htmlspecialchars($categoria[2]) . '</td>';
    
    // Descripción
    $html .= '<td>' . htmlspecialchars($categoria[3]) . '</td>';
    
    // Fecha Registro
    $html .= '<td>' . htmlspecialchars($categoria[4]) . '</td>';
...
$html .= '<div class="pie-pagina">
    Reporte generado por el sistema de gestión de categorías.<br>
...
header('Content-Disposition: attachment; filename="Reporte_Categorias_' . date('Ymd_His') . '.pdf"');
echo $dompdf->output();
exit;
?>