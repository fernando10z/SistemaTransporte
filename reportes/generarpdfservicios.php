...
$servicios = isset($_POST['filteredData']) ? json_decode($_POST['filteredData'], true) : [];
...
foreach ($servicios as $servicio) {
    if (strpos($servicio['estado'], 'Activo') !== false) {
        $serviciosActivos++;
    } else {
        $serviciosInactivos++;
    }
...
$html .= '<div style="margin-top: 15px; font-weight: bold; padding: 10px; border: 1px solid #ccc; border-radius: 5px; background-color: #f8f9fa;">
    <div style="display: flex; justify-content: space-between;">
        <div style="width: 32%; text-align: center; padding: 10px; background-color: #e9ecef; border-radius: 5px;">
            <strong>Total Servicios:</strong> ' . $totalServicios . '
        </div>
        <div style="width: 32%; text-align: center; padding: 10px; background-color: #e9ecef; border-radius: 5px;">
            <strong>Servicios Activos:</strong> ' . $serviciosActivos . '
        </div>
        <div style="width: 32%; text-align: center; padding: 10px; background-color: #e9ecef; border-radius: 5px;">
            <strong>Servicios Inactivos:</strong> ' . $serviciosInactivos . '
        </div>
    </div>
</div>';

// Sección del listado de servicios
$html .= '<div style="margin-top: 15px; font-weight: bold; background-color: #5d87ff; color: white; padding: 10px; border-radius: 5px;">LISTADO DE SERVICIOS FILTRADO</div>';
...
foreach ($servicios as $servicio) {
    ...
    // ID
    $html .= '<td>' . htmlspecialchars($servicio['id']) . '</td>';
    
    // Nombre
    $html .= '<td>' . htmlspecialchars($servicio['nombre']) . '</td>';
    
    // Descripción
    $html .= '<td>' . htmlspecialchars($servicio['descripcion']) . '</td>';
    
    // Tipo de Carga
    $html .= '<td>' . htmlspecialchars($servicio['tipo_carga']) . '</td>';
    
    // Estado
    $estado = trim($servicio['estado']);
    ...
    // Fecha Registro
    $html .= '<td>' . htmlspecialchars($servicio['fecha_registro']) . '</td>';
...
header('Content-Disposition: attachment; filename="Reporte_Servicios_' . date('Ymd_His') . '.pdf"');
echo $dompdf->output();
exit;
?>