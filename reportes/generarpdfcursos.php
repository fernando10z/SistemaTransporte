...
$cursos = isset($_POST['filteredData']) ? json_decode($_POST['filteredData'], true) : [];

// Verificar si los datos llegaron correctamente
if (!$cursos || empty($cursos)) {
    ...
    <tr>
        ...
        <td width="60%" style="text-align: center;">
            <div class="header">
                ...
                <p class="subtitle">Reporte de Cursos para Conductores</p>
            </div>
        </td>
...
foreach ($cursos as $curso) {
    if (strtolower($curso['estado']) === 'activado') {
        $cursosActivos++;
    } else {
        $cursosInactivos++;
    }
...
foreach ($cursos as $curso) {
    ...
    // ID
    $html .= '<td>' . htmlspecialchars($curso['idCurso']) . '</td>';
    
    // Nombre del Curso
    $html .= '<td>' . htmlspecialchars($curso['nombre_curso']) . '</td>';
    
    // Entidad
    $html .= '<td>' . htmlspecialchars($curso['entidad']) . '</td>';
    
    // Descripción
    $html .= '<td>' . htmlspecialchars($curso['descripcion']) . '</td>';
    
    // Fecha Registro
    $html .= '<td>' . htmlspecialchars($curso['fecharegistro']) . '</td>';
    
    // Estado
    $estado = trim($curso['estado']);
    if (strtolower($estado) === 'activado') {
        $html .= '<td class="text-center"><span class="badge badge-success">Activado</span></td>';
    } else {
        $html .= '<td class="text-center"><span class="badge badge-danger">Desactivado</span></td>';
    }
...
header('Content-Disposition: attachment; filename="Reporte_Cursos_' . date('Ymd_His') . '.pdf"');
echo $dompdf->output();
exit;
?>