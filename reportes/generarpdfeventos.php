...
$html = '<style>
    ...
    .stat-box { width: 19%; padding: 10px; background-color: #e9ecef; border-radius: 5px; text-align: center; }
</style>';
    ...
    <tr>
        ...
        <td width="60%" style="text-align: center;">
            <div class="header">
                ...
                <p class="subtitle">Reporte de Eventos de Ruta</p>
            </div>
        </td>
...
foreach ($eventos as $evento) {
    if ($evento['tipoPlanificacion'] === 'cliente') {
        $eventosClientes++;
    } else {
        $eventosEmpresas++;
    }
...
$html .= '<div class="stats-container">
    ...
    <div class="stat-box">
        <strong>Desvíos:</strong> ' . $estados['Desvío'] . '
    </div>
    <div class="stat-box">
        <strong>Retrasos:</strong> ' . $estados['Retraso'] . '
    </div>
</div>';

$html .= '<div class="stats-container">
    <div class="stat-box">
        <strong>Incidentes:</strong> ' . $estados['Incidente'] . '
    </div>
    <div class="stat-box">
        <strong>Completados:</strong> ' . $estados['Completado'] . '
    </div>
</div>';

// Sección del listado de eventos
$html .= '<div style="margin-top: 15px; font-weight: bold; background-color: #5d87ff; color: white; padding: 10px; border-radius: 5px;">LISTADO DE EVENTOS DE RUTA</div>';
...
foreach ($eventos as $evento) {
    ...
    // Tipo
    $tipo = $evento['tipoPlanificacion'] === 'cliente' ? 'Cliente' : 'Empresa';
    $html .= '<td>' . htmlspecialchars($tipo) . '</td>';
        $html .= '<td>' . htmlspecialchars($evento['nombre_cliente']) . '</td>';

    // Conductor
    $html .= '<td>' . htmlspecialchars($evento['conductor']) . '</td>';
    ...
    $badgeClass = match ($estado) {
        'Desvío'     => 'badge-warning',
        'Retraso'    => 'badge-danger',
        'Incidente'  => 'badge-dark',
...
header('Content-Disposition: attachment; filename="Reporte_Eventos_Ruta_' . date('Ymd_His') . '.pdf"');
echo $dompdf->output();
exit;
?>