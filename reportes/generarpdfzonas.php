...
$zonas = isset($_POST['filteredData']) ? json_decode($_POST['filteredData'], true) : [];
    ...
    <tr>
        ...
        <td width="60%" style="text-align: center;">
            <div class="header">
                ...
                <p class="subtitle">Reporte de Zonas de Cobertura</p>
            </div>
        </td>
        <td width="20%" style="text-align: right;">
            <div class="report-box">
                <h4>REPORTE DE ZONAS</h4>
                <h4>' . date('d/m/Y') . '</h4>
            </div>
        </td>
    </tr>
...
foreach ($zonas as $zona) {
    if (strpos($zona['estado'], 'Activo') !== false) {
        $zonasActivas++;
    } else {
        $zonasInactivas++;
    }
...
$html .= '<div style="margin-top: 15px; font-weight: bold; padding: 10px; border: 1px solid #ccc; border-radius: 5px; background-color: #f8f9fa;">
    <div style="display: flex; justify-content: space-between;">
        <div style="width: 32%; text-align: center; padding: 10px; background-color: #e9ecef; border-radius: 5px;">
            <strong>Total Zonas:</strong> ' . $totalZonas . '
        </div>
        <div style="width: 32%; text-align: center; padding: 10px; background-color: #e9ecef; border-radius: 5px;">
            <strong>Zonas Activas:</strong> ' . $zonasActivas . '
        </div>
        <div style="width: 32%; text-align: center; padding: 10px; background-color: #e9ecef; border-radius: 5px;">
            <strong>Zonas Inactivas:</strong> ' . $zonasInactivas . '
        </div>
    </div>
</div>';

// Sección del listado de zonas
$html .= '<div style="margin-top: 15px; font-weight: bold; background-color: #5d87ff; color: white; padding: 10px; border-radius: 5px;">LISTADO DE ZONAS FILTRADO</div>';
...
foreach ($zonas as $zona) {
    ...
    // ID
    $html .= '<td>' . htmlspecialchars($zona['id']) . '</td>';
    
    // Nombre Zona
    $html .= '<td>' . htmlspecialchars($zona['nombre']) . '</td>';
    
    // Departamento
    $html .= '<td>' . htmlspecialchars($zona['departamento']) . '</td>';
    
    // Provincia
    $html .= '<td>' . htmlspecialchars($zona['provincia']) . '</td>';
    
    // Distrito
    $html .= '<td>' . htmlspecialchars($zona['distrito']) . '</td>';
    
    // Descripción
    $html .= '<td>' . htmlspecialchars($zona['descripcion']) . '</td>';
    
    // Estado
    $estado = trim($zona['estado']);
    $badgeClass = strpos($estado, 'Activo') !== false ? 'badge-success' : 'badge-danger';
...
header('Content-Disposition: attachment; filename="Reporte_Zonas_' . date('Ymd_His') . '.pdf"');
echo $dompdf->output();
exit;
?>