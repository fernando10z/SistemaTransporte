...
$subcategorias = isset($_POST['filteredData']) ? json_decode($_POST['filteredData'], true) : [];

// Verificar si los datos llegaron correctamente
if (!$subcategorias || empty($subcategorias)) {
...
$html .= '<table width="100%">
    <tr>
        ...
        <td width="60%" style="text-align: center;">
            <div class="header">
                <h2 class="title">SISTEMA DE INVENTARIO</h2>
                <p class="subtitle">Reporte de SubCategorías</p>
            </div>
        </td>
        <td width="20%" style="text-align: right;">
            <div class="report-box">
                <h4>REPORTE DE SUBCATEGORÍAS</h4>
                <h4>' . date('d/m/Y') . '</h4>
            </div>
        </td>
    </tr>
...
$subcategoriasActivas = 0;
$subcategoriasInactivas = 0;
$almacenes = [];
$categorias = [];
...
foreach ($subcategorias as $subcategoria) {
    ...
    // Contar por almacén
    $almacen = $subcategoria['almacen'];
    if (!isset($almacenes[$almacen])) {
        $almacenes[$almacen] = 0;
    }
    $almacenes[$almacen]++;
...
$html .= '<div style="margin-top: 15px; font-weight: bold; background-color: #5d87ff; color: white; padding: 10px; border-radius: 5px;">DISTRIBUCIÓN POR CATEGORÍA</div>';
$html .= '<table class="table-container">
    <thead>
        <tr>
            <th>Categoría</th>
...
// Sección del listado de subcategorías
$html .= '<div style="margin-top: 15px; font-weight: bold; background-color: #5d87ff; color: white; padding: 10px; border-radius: 5px;">LISTADO DE SUBCATEGORÍAS FILTRADO</div>';
$html .= '<table class="table-container">
    <thead>
        <tr>
            <th>ID</th>
...
foreach ($subcategorias as $subcategoria) {
    ...
    // ID
    $html .= '<td class="text-center">' . htmlspecialchars($subcategoria['id']) . '</td>';
    
    // Almacén
    $html .= '<td>' . htmlspecialchars($subcategoria['almacen']) . '</td>';
    
    // Categoría
    $html .= '<td>' . htmlspecialchars($subcategoria['categoria']) . '</td>';
    
    // SubCategoría
    $html .= '<td>' . htmlspecialchars($subcategoria['subcategoria']) . '</td>';
    
    // Descripción
    $html .= '<td>' . htmlspecialchars($subcategoria['descripcion']) . '</td>';
    
    // Estado
    $badgeClass = $subcategoria['estado'] === 'Activo' ? 'badge-success' : 'badge-danger';
    $html .= '<td class="text-center"><span class="badge ' . $badgeClass . '">' . $subcategoria['estado'] . '</span></td>';
...
header('Content-Disposition: attachment; filename="Reporte_SubCategorias_' . date('Ymd_His') . '.pdf"');
echo $dompdf->output();
exit;
?>