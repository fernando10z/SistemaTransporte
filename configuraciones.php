...
<div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Configuración</h3>
        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="configuraciones-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Nombre Empresa"></th>
                    <th><input type="text" class="column-filter" placeholder="RUC"></th>
                ...
                <tbody>
                  ...
                  foreach ($configuraciones as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-configuracion'>{$row['id']}</span></td>";
                      ...
                      echo "<td><img src='configuracion/empresa/{$row['logo']}' alt='Logo' style='height: 50px;'></td>";
                  
                      // Botones de acción
                      echo "<td>";
                      echo "<div class='btn-action-wrapper'>";
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-configuracion' 
                              data-id='{$row['id']}'
                              title='Editar configuración'>
                              <i class='fas fa-pen'></i>
                            </button>";
                  
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-configuracion'
                              data-id='{$row['id']}'
...
<form id="pdfForm" action="reportes/generarpdfconfiguracion.php" method="post" style="display: none;">
 ...
 <?php include('configuracion/editar.php')?>
  <?php include('configuracion/modal.php')?>
  <script>
    function abrirModalConfiguraciones() {
        if (typeof $('#registrarConfiguracionModal').modal !== 'function') {
            console.error('Bootstrap Modal no está cargado correctamente');
            alert('Error al cargar el sistema. Recarga la página.');
            return;
        }
        
        $('#registrarConfiguracionModal').modal('show');
        
        const formRegistrarConfiguracion = document.getElementById('formRegistrarConfiguracion');
        if (formRegistrarConfiguracion) {
          formRegistrarConfiguracion.reset();
        } else {
            console.warn('Formulario con ID "formRegistrarConfiguracion" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#configuraciones-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-cog fa-3x text-muted mb-3"></i><p class="text-muted">No hay configuraciones para mostrar</p></div>'
    },
    pageLength: 10,
