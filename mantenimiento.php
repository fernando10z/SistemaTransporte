...
<div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Mantenimientos</h3>
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalMantenimiento()">
          ...
          <div class="filtros-avanzados" id="filtrosAvanzados">
            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label for="filtroTipo">Tipo de Mantenimiento</label>
                  <div class="select-container">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <div class="filtro-input">
                    ...
                    <input type="text" class="form-control" id="filtroBusquedaGlobal" placeholder="Buscar por placa, marca, modelo...">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="mantenimientos-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Placa</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Próximo"></th>
                    <th><input type="text" class="column-filter" placeholder="Km"></th>
                    <th><input type="text" class="column-filter" placeholder="Costo"></th>
                    <th><input type="text" class="column-filter" placeholder="Taller"></th>
                ...
                <tbody>
                  ...
                  $sql = "
                      SELECT 
                          ...
                          DATE_FORMAT(m.fecha_proxima_mantenimiento, '%d/%m/%Y') AS fecha_proxima_mantenimiento,
                          m.kilometraje,
                          m.costo,
                          m.taller,
                          m.observaciones,
                      ...
                      ORDER BY 
                          CASE WHEN m.estado = 'Pendiente' THEN 0
                              WHEN m.estado = 'Vencido' THEN 1
                              ELSE 2
                          END,
                          m.fecha_proxima_mantenimiento ASC";
                  ...
                  foreach ($mantenimientos as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-mantenimiento'>{$row['id']}</span></td>";
                      ...
                      echo "<td>{$row['fecha_proxima_mantenimiento']}</td>";
                      echo "<td>{$row['kilometraje']} km</td>";
                      echo "<td>S/ {$row['costo']}</td>";
                      ...
                      // Mostrar botón Editar solo si NO es Vencido
                      if ($row['estado'] !== 'Vencido') {
                          echo "<button class='btn btn-primary btn-icon btn-sm editar-mantenimiento me-1' 
                                  data-id='{$row['id']}' 
                                  title='Editar'>
                      ...
                      // Mostrar botón Cambiar Estado solo para Pendientes
                      if ($row['estado'] === 'Pendiente') {
                          echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-mantenimiento me-1' 
                                  data-id='{$row['id']}' 
                      ...
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-mantenimiento' 
                              data-id='{$row['id']}' 
                              title='Eliminar'>
...
<form id="pdfForm" action="reportes/generarpdfmantenimiento.php" method="post" style="display: none;">
...
$(document).ready(function() {
  ...
  var table = $('#mantenimientos-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-tools fa-3x text-muted mb-3"></i><p class="text-muted">No hay mantenimientos registrados</p></div>'
    },
    pageLength: 10,
    ...
    initComplete: function() {
      ...
      this.api().columns().every(function(colIdx) {
        if (colIdx === 13) return; // No aplicar filtro a la columna de acciones
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      ...
      filteredData.push({
        id: $row.find('.id-mantenimiento').text(),
        placa: rowData[1],
        marca: rowData[2],
        modelo: rowData[3],
        tipo_mantenimiento: rowData[4],
        descripcion: rowData[5],
        fecha_mantenimiento: rowData[6],
        fecha_proxima_mantenimiento: rowData[7],
        kilometraje: rowData[8],
        costo: rowData[9],
        taller: rowData[10],
        observaciones: rowData[11],
        estado: $(rowData[12]).text().trim()
      });
    });
...
$(document).ready(function () {
  ...
  /* ---------- Cambiar estado ---------- */
  $(document).on('click', '.cambiar-estado-mantenimiento', function () {
    const id   = $(this).data('id');

    Swal.fire({
      title: '¿Cambiar estado?',
      text: 'El mantenimiento avanzará al siguiente estado lógico.',
      icon: 'question',
      ...
      if (result.isConfirmed) {
        $.post('mantemientovehiculos/cambiarestado.php', { id }, respuesta => {
          if (respuesta.ok) {
  ...
  $(document).on('click', '.eliminar-mantenimiento', function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.post('mantemientovehiculos/eliminar.php', { id }, respuesta => {
          if (respuesta.ok) {
...
<script>
$(document).ready(function() {
    // Verificar mantenimientos pendientes al cargar la página
    verificarMantenimientosPendientes();
    
    async function verificarMantenimientosPendientes() {
        try {
            const response = await $.ajax({
                url: 'mantemientovehiculos/verificarmantenimiento.php',
                method: 'GET',
                dataType: 'json'
            });
            ...
            // Separar alertas urgentes y avisos
            const urgentes = response.filter(a => a.tipo === 'urgente');
            const avisos = response.filter(a => a.tipo === 'aviso');
            
            // Mostrar primero alertas urgentes
            if (urgentes.length > 0) {
                await mostrarAlertaMantenimiento({
                    tipo: 'urgente',
                    items: urgentes,
                    titulo: '¡MANTENIMIENTOS URGENTES DEBES ACTUALIZAR!'
                });
            }
            
            // Mostrar luego alertas normales
            if (avisos.length > 0) {
                await mostrarAlertaMantenimiento({
                    tipo: 'aviso',
                    items: avisos,
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Ocurrió un error al verificar mantenimientos',
                confirmButtonText: 'Entendido'
            });
        }
    }
    
    function mostrarAlertaMantenimiento(opciones) {
        return new Promise((resolve) => {
            // Crear contenido HTML
            let html = `
                <div class="text-start">
                    <p><i class="${opciones.tipo === 'urgente' ? 'fas fa-exclamation-triangle' : 'fas fa-info-circle'}"></i> 
            ...
            opciones.items.forEach(item => {
                const estiloFila = item.tipo === 'urgente' ? 'style="color: #dc3545; font-weight: bold;"' : '';
                const textoDias = item.dias < 0 ? `Vencido hace ${Math.abs(item.dias)} días` : 
                                 (item.dias === 0 ? 'HOY' : `En ${item.dias} días`);
            ...
            $(document).on('click', '.editar-mantenimiento-alerta', function() {
                const id = $(this).data('id');
                // Aquí puedes implementar la lógica para editar el mantenimiento
                alerta.close();
    ...
    function editarMantenimiento(id) {
        // Implementa tu lógica para abrir el modal de edición
        console.log('Editar mantenimiento ID:', id);
        // Ejemplo: $('#modalEditarMantenimiento').modal('show');
        // y cargar los datos del mantenimiento con este ID
    }
});
</script>
<style>
...
.alerta-urgente tr:last-child td,
.alerta-aviso tr:last-child td {
    border-bottom: none;
}
