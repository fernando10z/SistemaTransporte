        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalTarifas()">
            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <div class="select-container">
                    <select class="form-control" id="filtroServicio">
                      <option value="">Todos</option>
                      <?php
                      // Obtener servicios únicos de la base de datos
                      $sqlServicios = "SELECT idServicio, nombreServicio FROM servicios";
                      $stmtServicios = $conn->query($sqlServicios);
                      while ($servicio = $stmtServicios->fetch(PDO::FETCH_ASSOC)) {
                          echo "<option value='{$servicio['nombreServicio']}'>{$servicio['nombreServicio']}</option>";
                      }
                      ?>
                    </select>
                  </div>
                ...
                <div class="form-group">
                  ...
                  <div class="select-container">
                    <select class="form-control" id="filtroZona">
                      ...
                      $sqlZonas = "SELECT idZona, nombreZona FROM zonas_cobertura";
                      $stmtZonas = $conn->query($sqlZonas);
                      while ($zona = $stmtZonas->fetch(PDO::FETCH_ASSOC)) {
                          echo "<option value='{$zona['nombreZona']}'>{$zona['nombreZona']}</option>";
                      }
                      ?>
                    </select>
                  </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="tarifas-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Servicio</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Servicio"></th>
                    <th><input type="text" class="column-filter" placeholder="Zona"></th>
                    ...
                    <th><input type="text" class="column-filter" placeholder="Vigencia"></th>
                ...
                <tbody>
                  ...
                  foreach ($tarifas as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-tarifa'>{$row['id']}</span></td>";
                      echo "<td>{$row['nombreServicio']}</td>";
                      echo "<td>{$row['nombreZona']}</td>";
                      echo "<td class='text-end'>" . number_format($row['monto'], 2) . "</td>";
                      echo "<td>{$row['observaciones']}</td>";
                      echo "<td>" . date('d/m/Y', strtotime($row['fechaVigencia'])) . "</td>";
                      ...
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-tarifa me-1' 
                              data-id='{$row['id']}' title='Editar'>
                              <i class='fas fa-pen'></i>
                            </button>";
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-tarifa me-1' 
                              data-id='{$row['id']}' title='Eliminar'>
                              <i class='fas fa-trash'></i>
                            </button>";
                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-tarifa' 
                              data-id='{$row['id']}' 
                              data-estado='{$row['Estado']}' title='Cambiar estado'>
...
<form id="pdfForm" action="reportes/generarpdftarifa.php" method="post" style="display: none;">
...
<?php include('tarifa/modal.php')?>
  <?php include('tarifa/modaleditar.php')?>
  
  <script>
    function abrirModalTarifas() {
        if (typeof $('#tarifaModal').modal !== 'function') {
        ...
        if (tarifaForm) {
          tarifaForm.reset();
            ...
            console.warn('Formulario con ID "tarifaForm" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#tarifas-table').DataTable({
    responsive: true,
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-money-bill-wave fa-3x text-muted mb-3"></i><p class="text-muted">No hay tarifas registradas</p></div>'
    },
    pageLength: 10,
    ...
    initComplete: function() {
      ...
      this.api().columns().every(function(colIdx) {
        if (colIdx === 7) return; // No aplicar filtro a la columna de acciones
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var row = this.node();
      var data = {
        id: $(row).find('.id-tarifa').text(),
        servicio: $(row).find('td:eq(1)').text(),
        zona: $(row).find('td:eq(2)').text(),
        monto: $(row).find('td:eq(3)').text(),
        observaciones: $(row).find('td:eq(4)').text(),
        fechaVigencia: $(row).find('td:eq(5)').text(),
        estado: $(row).find('td:eq(6)').text().trim()
      };
...
<script>
// ✅ Eliminar tarifa con SweetAlert y recarga
$(document).on('click', '.eliminar-tarifa', function () {
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    Swal.fire({
                        icon: res.status,
                        title: res.status === 'success' ? '¡Eliminado!' : 'Error',
                        text: res.message,
                        timer: 1000,
...
// ✅ Cambiar estado con SweetAlert y recarga
$(document).on('click', '.cambiar-estado-tarifa', function () {
    const id = $(this).data('id');

    $.ajax({
        url: 'tarifa/cambiarestado.php',
        type: 'POST',
        data: { id },
        success: function (response) {
            const res = JSON.parse(response);
            Swal.fire({
                icon: res.status,
                title: res.status === 'success' ? '¡Estado actualizado!' : 'Error',
                text: res.status === 'success' ? 'Nuevo estado: ' + res.nuevoEstado : res.message,
                timer: 1000,
                showConfirmButton: false
            });
            if (res.status === 'success') {
...
<script>
$(document).ready(function() {
    // Verificar tarifas por vencer al cargar la página
    verificarTarifasPorVencer();
    
    async function verificarTarifasPorVencer() {
        try {
            const response = await $.ajax({
                url: 'tarifa/verificarfecha.php',
                method: 'GET',
                dataType: 'json'
            });
            ...
            const urgentes = response.data.filter(t => t.prioridad === 'urgente');
            const avisos = response.data.filter(t => t.prioridad === 'aviso');
            
            // Mostrar alertas urgentes primero (hoy y mañana)
            if (urgentes.length > 0) {
                await mostrarAlertaTarifas({
                    tipo: 'urgente',
                    items: urgentes,
                    titulo: '¡TARIFAS QUE VENCEN HOY O MAÑANA!'
                });
            }
            
            // Mostrar avisos normales después (2-5 días)
            if (avisos.length > 0) {
                await mostrarAlertaTarifas({
                    tipo: 'aviso',
                    items: avisos,
                    titulo: 'Tarifas por vencer en los próximos días'
                });
            }
    ...
    function mostrarAlertaTarifas(opciones) {
        return new Promise((resolve) => {
            let html = `
                <div class="text-start">
                    <p class="${opciones.tipo === 'urgente' ? 'text-danger' : 'text-warning'}">
                        <i class="fas ${opciones.tipo === 'urgente' ? 'fa-exclamation-circle' : 'fa-info-circle'}"></i> 
                        <strong>${opciones.titulo}</strong>
                    </p>
                    <div class="alert-container" style="max-height: 300px; overflow-y: auto;">
                        <table class="table table-sm">
                            <thead>
                                <tr class="${opciones.tipo === 'urgente' ? 'bg-danger text-white' : 'bg-warning'}">
                                    <th>Servicio</th>
                                    <th>Zona</th>
                                    <th>Vence</th>
                                    <th>Monto</th>
            ...
            opciones.items.forEach(item => {
                ...
                if (item.dias === 0) {
                    ...
                    estado = '<span class="badge bg-danger">MAÑANA</span>';
                } else {
                    estado = `<span class="badge bg-warning text-dark">En ${item.dias} días</span>`;
                }
                
                html += `
                    <tr>
                        <td>
                            <strong>${item.nombreServicio}</strong>
                            <small class="text-muted d-block">${item.tipoCarga}</small>
                        </td>
                        <td>
                            ${item.nombreZona}
                            <small class="text-muted d-block">${item.departamento}</small>
                        </td>
                        <td>
                            ${item.fechaVigencia}
            ...
            Swal.fire({
                icon: opciones.tipo === 'urgente' ? 'error' : 'warning',
    ...
    $(document).on('click', '.editar-tarifa-alerta', function() {
        const idTarifa = $(this).data('id');
        $('#editarTarifaModal').modal('show');
        // Aquí puedes cargar los datos de la tarifa en el modal de edición
    });
});
</script>
</body>
