...
<div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Vehículos</h3>
        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalVehiculos()">
                ...
                <div class="form-group">
                  ...
                  <div class="select-container">
                    <select class="form-control" id="filtroMarca">
                      ...
                      <?php
                      $sqlMarcas = "SELECT DISTINCT marca FROM vehiculos WHERE marca IS NOT NULL ORDER BY marca";
                ...
                <div class="form-group">
                  ...
                  <div class="select-container">
                    <select class="form-control" id="filtroModelo">
                      <option value="">Todos</option>
                      <?php
                      $sqlModelos = "SELECT DISTINCT modelo FROM vehiculos WHERE modelo IS NOT NULL ORDER BY modelo";
                      $stmtModelos = $conn->prepare($sqlModelos);
                      $stmtModelos->execute();
                      $modelos = $stmtModelos->fetchAll(PDO::FETCH_COLUMN);
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="vehiculos-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Placa</th>
                ...
                <tbody>
                    ...
                    foreach ($vehiculos as $row) {
                        echo "<tr>";
                        echo "<td><span class='id-vehiculo'>{$row['id']}</span></td>";
                        echo "<td>{$row['placa']}</td>";
                        echo "<td>{$row['marca']}</td>";
                        echo "<td>{$row['modelo']}</td>";
                        echo "<td class='text-end'>{$row['capacidadPeso']} kg</td>";
                        echo "<td class='text-end'>{$row['capacidadVolumen']} m³</td>";

                        // Badge del estado
                        if ($row['estado'] === 'Disponible') {
                            echo '<td class="text-center"><span class="badge badge-success"><i class="fas fa-check-circle me-1"></i> Disponible</span></td>';
                        } else {
                            echo '<td class="text-center"><span class="badge badge-danger"><i class="fas fa-times-circle me-1"></i> Ocupado</span></td>';
                        }

                        // Botones de acción
                        echo "<td class='text-center'>";
                        echo "<div class='btn-action-wrapper'>";
                        echo "<button class='btn btn-primary btn-icon btn-sm editar-vehiculo me-1'
                                data-id='{$row['id']}' 
                                title='Editar vehículo'>
                                <i class='fas fa-pen'></i>
                              </button>";

                        echo "<button class='btn btn-danger btn-icon btn-sm eliminar-vehiculo me-1'
                                data-id='{$row['id']}' 
                                title='Eliminar vehículo'>
                                <i class='fas fa-trash'></i>
                              </button>";

                        echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-vehiculo'
                                data-id='{$row['id']}' 
...
<form id="pdfForm" action="reportes/generarpdfvehiculos.php" method="post" style="display: none;">
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <?php include('vehiculos/modal.php')?>
  <?php include('vehiculos/modaleditar.php')?>
  
  <script>
    function abrirModalVehiculos() {
        if (typeof $('#registrarVehiculoModal').modal !== 'function') {
            console.error('Bootstrap Modal no está cargado correctamente');
            alert('Error al cargar el sistema. Recarga la página.');
            return;
        }
        
        $('#registrarVehiculoModal').modal('show');
        
        const formRegistroVehiculo = document.getElementById('formRegistroVehiculo');
        if (formRegistroVehiculo) {
          formRegistroVehiculo.reset();
        } else {
            console.warn('Formulario con ID "formRegistroVehiculo" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#vehiculos-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-truck fa-3x text-muted mb-3"></i><p class="text-muted">No hay vehículos registrados</p></div>'
    },
    pageLength: 10,
    ...
    columnDefs: [
      { 
        targets: [4, 5], // Columnas de capacidad
        className: 'text-end'
      },
      { 
        targets: 6, // Estado
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var row = this.node();
      var data = {
        id: $(row).find('.id-vehiculo').text(),
        placa: $(row).find('td:eq(1)').text(),
        marca: $(row).find('td:eq(2)').text(),
        modelo: $(row).find('td:eq(3)').text(),
        capacidadPeso: $(row).find('td:eq(4)').text(),
        capacidadVolumen: $(row).find('td:eq(5)').text(),
...
$(document).ready(function () {
    // CAMBIAR ESTADO
    $(".cambiar-estado-vehiculo").on("click", function () {
        const id = $(this).data("id");

        Swal.fire({
            title: '¿Cambiar estado?',
            text: "¿Deseas cambiar el estado de este vehículo?",
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.post("vehiculos/cambiarestado.php", { id }, function (response) {
                    Swal.fire({
                        icon: 'success',
                        title: response,
    ...
    // ELIMINAR VEHÍCULO
    $(".eliminar-vehiculo").on("click", function () {
        const id = $(this).data("id");
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post("vehiculos/eliminar.php", { id }, function (response) {
                    Swal.fire({
                        icon: 'success',
                        title: response,
                        showConfirmButton: false,
