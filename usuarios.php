        ...
        <div class="card">
                ...
                <div class="form-group">
                  ...
                  <div class="select-container">
                    <select class="form-control" id="filtroRol">
                      ...
                      <?php
                      $sqlRoles = "SELECT idRol, nombreRol FROM roles";
                      $stmtRoles = $conn->prepare($sqlRoles);
                      $stmtRoles->execute();
                      $roles = $stmtRoles->fetchAll(PDO::FETCH_ASSOC);
                      
                      foreach ($roles as $rol) {
                          echo "<option value='{$rol['nombreRol']}'>{$rol['nombreRol']}</option>";
                      }
                      ?>
                    </select>
                  </div>
                ...
                <div class="form-group">
                  ...
                  <div class="select-container">
                    <select class="form-control" id="filtroGenero">
                      <option value="">Todos</option>
                      <?php
                      $sqlGeneros = "SELECT idGenero, genero FROM genero";
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="usuarios-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Rol"></th>
                ...
                <tbody>
                  ...
                  $sql = "
                      ...
                      LEFT JOIN tipodireccion td ON u.idTipoDireccion = td.idTipoDireccion
                      LEFT JOIN tipodocumento td2 ON u.idTipoDocumento = td2.idTipoDocumento
                  ...
                  foreach ($usuarios as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-usuario'>{$row['idUsuario']}</span></td>";
                      echo "<td>{$row['nombreCompleto']} {$row['apellidos']}</td>";
                      ...
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-usuario' 
                              data-id='{$row['idUsuario']}'
                              title='Editar usuario'>
                              <i class='fas fa-pen'></i>
                            </button>";
                      
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-usuario'
                              data-id='{$row['idUsuario']}'
                              title='Eliminar usuario'>
                              <i class='fas fa-trash'></i>
                            </button>";
                      
                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-usuario'
                              data-id='{$row['idUsuario']}'
...
<form id="pdfForm" action="reportes/generarpdfusuarios.php" method="post" style="display: none;">
...
$(document).ready(function() {
    ...
    <?php include('usuarios/modal.php')?>
        ...
        <?php include('usuarios/modaleditar.php')?>
<script>
$(document).ready(function() {
  ...
  var table = $('#usuarios-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-users fa-3x text-muted mb-3"></i><p class="text-muted">No hay usuarios registrados</p></div>'
    },
    pageLength: 10,
    ...
    initComplete: function() {
      ...
      $('#filtroGenero').on('change', function() {
        table.column(2).search(this.value).draw(); // Ajustar índice según la columna de género
      });
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var row = this.node();
      var data = {
        id: $(row).find('.id-usuario').text(),
        nombre: $(row).find('td:eq(1)').text(),
        correo: $(row).find('td:eq(2)').text(),
        direccion: $(row).find('td:eq(3)').text(),
        telefono: $(row).find('td:eq(4)').text(),
        documento: $(row).find('td:eq(5)').text(),
        rol: $(row).find('td:eq(6)').text(),
...
$(document).ready(function() {
    ...
    // Cambiar estado
    $('.cambiar-estado-usuario').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('usuarios/cambiarestado.php', { idUsuario: id, nuevoEstado: nuevoEstado }, function(response) {
    ...
    $('.eliminar-usuario').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('usuarios/eliminar.php', { idUsuario: id }, function(response) {
