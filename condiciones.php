 ...
 <div class="container-fluid">
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Condiciones</h3>
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Condiciones</li>
          </ol>
        </nav>
      </div>
    </div>
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="card-title">Listado de Términos y Condiciones</h4>
            <div class="d-flex">
              <button class="btn btn-toggle-filtros me-2" id="toggleFiltros">
                <i class="fas fa-filter"></i>
              </button>
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalCondiciones()">
              ...
              <div class="col-md-8">
                <div class="form-group">
                  ...
                  <div class="filtro-input">
                    ...
                    <input type="text" class="form-control" id="filtroBusquedaGlobal" placeholder="Buscar por título, contenido, etc...">
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="condiciones-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Título</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Título"></th>
                    <th><input type="text" class="column-filter" placeholder="Contenido"></th>
                    <th><input type="text" class="column-filter" placeholder="Orden"></th>
                ...
                <tbody>
                  ...
                  foreach ($terminos as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-termino'>{$row['id_terminos']}</span></td>";
                      echo "<td>{$row['titulo']}</td>";
                      echo "<td>" . substr(strip_tags($row['contenido']), 0, 60) . "...</td>";
                      ...
                      echo "<button class='btn btn-primary btn-icon btn-sm btnEditarTermino' 
                              data-id='{$row['id_terminos']}' 
                              title='Editar término'>
                              <i class='fas fa-pen'></i>
                            </button>";
                  
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-termino'
                              data-id='{$row['id_terminos']}'
...
<form id="pdfForm" action="reportes/generarpdfcondiciones.php" method="post" style="display: none;">
 ...
 <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  ...
  <?php include('condiciones/modal.php')?>
  <?php include('condiciones/modaleditar.php')?>
  
  <script>
    function abrirModalCondiciones() {
        if (typeof $('#modalRegistrarTermino').modal !== 'function') {
            console.error('Bootstrap Modal no está cargado correctamente');
            alert('Error al cargar el sistema. Recarga la página.');
            return;
        }
        
        $('#modalRegistrarTermino').modal('show');
        
        const formRegistrarTermino = document.getElementById('formRegistrarTermino');
        if (formRegistrarTermino) {
          formRegistrarTermino.reset();
        } else {
            console.warn('Formulario con ID "formRegistrarTermino" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#condiciones-table').DataTable({
    responsive: true,
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-file-contract fa-3x text-muted mb-3"></i><p class="text-muted">No hay términos y condiciones para mostrar</p></div>'
    },
    pageLength: 10,
    ...
    initComplete: function() {
      ...
      this.api().columns().every(function(colIdx) {
        if (colIdx === 5) return; // No aplicar filtro a la columna de acciones
...
<script>
$(document).on('click', '.eliminar-termino', function () {
