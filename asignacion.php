...
<head>
  ...
  <title>Gestión de Asignacion</title>
...
<body>
  ...
  <div class="page-inner">
    <div class="page-header">
      <div>
        <h3 class="page-title">Gestión de Asignaciones</h3>
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="card-title">Listado de Asignaciones</h4>
            <div class="d-flex">
              <button class="btn btn-toggle-filtros me-2" id="toggleFiltros">
                <i class="fas fa-filter"></i>
              </button>
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalAsignacion()">
            ...
            <div class="row">
              <div class="col-md-3">
                <div class="form-group">
                  <label for="filtroTipo">Tipo de Asignaciones</label>
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="asignaciones-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    ...
                    <th>Origen</th>
                    <th>Destino</th>
                    <th width="90px">Peso</th>
                    <th width="100px">Volumen</th>
                    <th width="100px">Costo</th>
                    <th width="150px">Código Seguimiento</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Vehículo"></th>
                    ...
                    <th><input type="text" class="column-filter" placeholder="Peso"></th>
                    <th><input type="text" class="column-filter" placeholder="Volumen"></th>
                ...
                <tbody>
                  ...
                  $sql = "
                  SELECT 
                      ac.idAsignacion AS idAsignacion,
                      ...
                      sc.volumen,
                      cc.montoFinal,
                      ac.estado AS estadoAsignacion,
                      ac.fechaAsignacion,
                      ac.observaciones,
                      IFNULL(sec.codigoSeguimiento, 'No generado') AS codigoSeguimiento
                  ...
                  SELECT 
                      ...
                      se.volumen,
                      ce.montoFinal,
                      ace.estado AS estadoAsignacion,
                      ace.fechaAsignacion,
                      ace.observaciones,
                      IFNULL(see.codigoSeguimiento, 'No generado') AS codigoSeguimiento
                  ...
                  foreach ($asignaciones as $row) {
                      ...
                      // Tipo de cliente con icono
                      if ($row['tipoCliente'] == 'Natural') {
                          echo "<td><i class='fas fa-user me-2 text-primary'></i> Natural</td>";
                      } else {
                          echo "<td><i class='fas fa-building me-2 text-info'></i> Empresa</td>";
                      }
                      
                      echo "<td>{$row['cliente']}</td>";
                      echo "<td>{$row['vehiculo_placa']} - {$row['vehiculo_modelo']}</td>";
                      echo "<td>{$row['origen']}</td>";
                      echo "<td>{$row['destino']}</td>";
                      echo "<td>{$row['peso']} kg</td>";
                      echo "<td>{$row['volumen']} m³</td>";
                      echo "<td>S/ {$row['montoFinal']}</td>";
                      echo "<td>{$row['codigoSeguimiento']}</td>";
                      
                      // Estado de la asignación con badge
                      $badgeClass = match($row['estadoAsignacion']) {
                          'Pendiente' => 'warning',
                          'En tránsito' => 'info',
                          'Entregado' => 'success',
                          default => 'secondary'
                      };
                      echo "<td><span class='badge bg-{$badgeClass}'><i class='fas fa-circle me-1'></i> {$row['estadoAsignacion']}</span></td>";
                      
                      // Botones de acción
                      echo "<td class='text-center'>";
                      echo "<div class='btn-action-wrapper'>";
                      $disabled = $row['estadoAsignacion'] == 'Entregado' ? 'disabled' : '';

                    // Botón Editar
                    echo "<button class='btn btn-primary btn-icon btn-sm editar' 
                            data-id='{$row['idAsignacion']}' 
                            data-type='{$row['tipoAsignacion']}'
                            title='Editar' $disabled>
                            <i class='fas fa-edit'></i>
                          </button>";

                    // Botón Cambiar estado
                    echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado'
                            data-id='{$row['idAsignacion']}'
                            data-type='{$row['tipoAsignacion']}'
                            title='Cambiar estado' $disabled>
                            <i class='fas fa-sync-alt'></i>
                          </button>";
                      echo "<button class='btn btn-info btn-icon btn-sm generar-qr'
                              data-id='{$row['idAsignacion']}'
                              data-type='{$row['tipoAsignacion']}'
                              data-cliente='".htmlspecialchars($row['cliente'], ENT_QUOTES)."'
                              data-vehiculo='{$row['vehiculo_placa']} - {$row['vehiculo_modelo']}'
                              data-peso='{$row['peso']} kg'
                              data-volumen='{$row['volumen']} m³'
                              data-monto='S/ {$row['montoFinal']}'
                      ...
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar'
                              data-id='{$row['idAsignacion']}'
                              data-type='{$row['tipoAsignacion']}'
                              title='Eliminar'>
...
<form id="pdfForm" action="reportes/generarpdfasignacion.php" method="post" style="display: none;">
 ...
 <?php include('asignacion/modal.php')?>
  
  <?php include('asignacion/modaleditar.php')?>
  <?php include('generarqr/modal.php')?>
  <script src="https://cdn.jsdelivr.net/npm/qrcode@1.5.1/build/qrcode.min.js"></script>
<script>
  $(document).on('click', '.generar-qr', function () {
    const idAsignacion = $(this).data('id');
    const tipoAsignacion = $(this).data('type');
    ...
    $('#qrCliente').text(cliente);
    $('#qrDocumento').text(documento);
    $('#qrVehiculo').text(vehiculo);
    $('#qrCarga').text(carga);
    $('#qrPeso').text(peso);
    $('#qrVolumen').text(volumen);
    $('#qrMonto').text(monto);
    $('#qrOrigen').text(origen);
    $('#qrDestino').text(destino);

    // Petición AJAX para crear el seguimiento
    $.ajax({
        url: 'seguimiento/crearseguimiento.php',
        method: 'POST',
        data: {
        ...
        success: function (response) {
            ...
            try {
                ...
                codigoSeguimiento = res.codigoSeguimiento || 'N/A';
            } catch (e) {
            ...
            // Limpiar y crear canvas QR
            $('#qrCodeContainer').empty();
            const qrCanvas = $('<canvas id="qrCodeCanvas"></canvas>');
            $('#qrCodeContainer').append(qrCanvas);

            // Generar QR
            QRCode.toCanvas(document.getElementById("qrCodeCanvas"), JSON.stringify(qrData), {
                width: 200,
                margin: 2,
                color: {
                    dark: '#000000',
            ...
            // Botón de descarga
            $('#descargarQR').off('click').on('click', function () {
                ...
                link.download = `qr_envio_${idAsignacion}.png`;
        ...
        error: function () {
            console.error("Error al registrar el seguimiento.");
        }
    });
});
...
<script>
    function abrirModalAsignacion() {
        if (typeof $('#asignacionModal').modal !== 'function') {
        ...
        if (asignacionForm) {
          asignacionForm.reset();
            ...
            console.warn('Formulario con ID "asignacionForm" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#asignaciones-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-boxes fa-3x text-muted mb-3"></i><p class="text-muted">No hay asignaciones para mostrar</p></div>'
    },
    pageLength: 5, // Mostrar solo 5 registros por página inicialmente
      ...
      $(document).on('click', '.cambiar-estado', function() {
          ...
          Swal.fire({
              ...
              if (result.isConfirmed) {
                  ...
                  $.ajax({
                      url: 'asignacion/cambiarestado.php',
                      type: 'POST',
                      data: {
                          id: id,
                          tipo: tipo,
