        ...
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
              ...
              <div class="btn-group ml-2">
                <button class="btn btn-primary" onclick="abrirModalSolicitudes()">
          ...
          <div class="filtros-avanzados" id="filtrosAvanzados">
            ...
            <div class="row">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label for="filtroTipoCarga">Tipo de Carga</label>
                  <div class="select-container">
                    <select class="form-control" id="filtroTipoCarga">
                      <option value="">Todos</option>
                      <?php
                      // Obtener tipos de carga únicos de la base de datos
                      $sqlTipos = "SELECT DISTINCT tipoCarga FROM solicitudes_clientes WHERE tipoCarga IS NOT NULL AND tipoCarga != ''
                                   UNION
                                   SELECT DISTINCT tipo_carga FROM solicitud_empresa WHERE tipo_carga IS NOT NULL AND tipo_carga != ''";
                      $stmtTipos = $conn->prepare($sqlTipos);
                      $stmtTipos->execute();
                      $tiposCarga = $stmtTipos->fetchAll(PDO::FETCH_COLUMN);
          ...
          <div class="card-body">
            <div class="table-responsive table-fade-in">
              <table id="solicitudes-table" class="display table table-hover w-100">
                <thead>
                  <tr>
                    <th width="70px">ID</th>
                    <th>Tipo Cliente</th>
                  ...
                  <tr class="filter-header">
                    ...
                    <th><input type="text" class="column-filter" placeholder="Carga"></th>
                ...
                <tbody>
                  ...
                  SELECT 
                      ...
                      e.destino,
                      e.estado,
                      e.fecha_solicitud AS fechaRegistro
                  ...
                  foreach ($solicitudes as $row) {
                      echo "<tr>";
                      echo "<td><span class='id-solicitud'>{$row['idSolicitud']}</span></td>";
                      ...
                      // Mostrar nombre o razón social
                      $cliente = $row['tipo'] === 'Natural' ? $row['nombreCliente'] : $row['razonSocial'];
                      echo "<td>{$cliente}</td>";
                      ...
                      $badgeConfig = [
                          'Pendiente' => ['class' => 'warning', 'icon' => 'fa-clock'],
                          'Cotizado' => ['class' => 'info', 'icon' => 'fa-file-invoice-dollar'],
                          'Asignado' => ['class' => 'primary', 'icon' => 'fa-truck'],
                          'Entregado' => ['class' => 'success', 'icon' => 'fa-check-circle'],
                          'Anulado' => ['class' => 'danger', 'icon' => 'fa-times-circle']
                      ];
                      
                      $estado = ucfirst(strtolower($row['estado']));
                      $badgeClass = $badgeConfig[$estado]['class'] ?? 'secondary';
                      $badgeIcon = $badgeConfig[$estado]['icon'] ?? 'fa-question';
                      
                      echo "<td><span class='badge bg-{$badgeClass}'><i class='fas {$badgeIcon} me-1'></i> {$estado}</span></td>";
                      
                      echo "<td>" . date('d/m/Y H:i', strtotime($row['fechaRegistro'])) . "</td>";
                      
                      // Botones de acción
                      echo "<td>";
                      echo "<div class='btn-action-wrapper'>";
                      echo "<button class='btn btn-primary btn-icon btn-sm editar-solicitud me-1' 
                      ...
                      echo "<button class='btn btn-danger btn-icon btn-sm eliminar-solicitud me-1'
                              data-id='{$row['idSolicitud']}'
                              data-tipo='{$row['tipo']}'
                      ...
                      echo "<button class='btn btn-warning btn-icon btn-sm cambiar-estado-solicitud'
                              data-id='{$row['idSolicitud']}'
                              data-tipo='{$row['tipo']}'
...
<form id="pdfForm" action="reportes/generarpdfsolicitudes.php" method="post" style="display: none;">
  ...
  <?php include('solicitudes/modal.php')?>
  <?php include('solicitudes/modaleditar.php')?>
  
  <script>
    function abrirModalSolicitudes() {
        if (typeof $('#registroSolicitudModal').modal !== 'function') {
            console.error('Bootstrap Modal no está cargado correctamente');
            alert('Error al cargar el sistema. Recarga la página.');
            return;
        }
        
        $('#registroSolicitudModal').modal('show');
        
        const formSolicitud = document.getElementById('formSolicitud');
        if (formSolicitud) {
          formSolicitud.reset();
        } else {
            console.warn('Formulario con ID "formSolicitud" no encontrado');
        }
    }
...
$(document).ready(function() {
  ...
  var table = $('#solicitudes-table').DataTable({
    ...
    language: {
      ...
      emptyTable: '<div class="text-center my-5"><i class="fas fa-file-alt fa-3x text-muted mb-3"></i><p class="text-muted">No hay solicitudes registradas</p></div>'
    },
    pageLength: 10,
    ...
    initComplete: function() {
      ...
      $('#filtroTipoCarga').on('change', function() {
  ...
  $('#pdfBtn').on('click', function() {
    ...
    table.rows({search: 'applied'}).every(function(rowIdx) {
      var row = this.node();
      var data = {
        id: $(row).find('.id-solicitud').text(),
        tipo_cliente: $(row).find('td:eq(1)').text().trim(),
        cliente: $(row).find('td:eq(2)').text(),
        tipo_carga: $(row).find('td:eq(3)').text(),
        peso: $(row).find('td:eq(4)').text(),
        volumen: $(row).find('td:eq(5)').text(),
        origen: $(row).find('td:eq(6)').text(),
        destino: $(row).find('td:eq(7)').text(),
        estado: $(row).find('td:eq(8)').text().trim(),
        fecha: $(row).find('td:eq(9)').text()
      };
...
$(document).on('click', '.eliminar-servicio', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('servicio/eliminar.php', { id }, function (response) {
...
$(document).on('click', '.cambiar-estado-servicio', function () {
    const id = $(this).data('id');

    $.post('servicio/cambiarestado.php', { id }, function (response) {
        const res = JSON.parse(response);
